					...
					SwarmEvent::Behaviour(event) => match event {
						...
						CoreEvent::Ping(ping::Event {
							...
							match result {
								// Inbound ping succes
								Ok(duration) => {
									// In handling the ping error policies, we only bump up an error
									// count when there is CONCURRENT failure. If the peer becomes
									// responsive, its recorded error count decays by 50% on every
									// success, until it gets to 1
								...
								Err(err_type) => {
									...
									match network_info.ping.policy {
										PingErrorPolicy::NoDisconnect => {
											// Do nothing, we can't disconnect from peer under any
											// circumstances
										},
										PingErrorPolicy::DisconnectAfterMaxErrors(max_errors) => {
											// Disconnect after we've recorded a certain number of
											// concurrent errors
											...
											if *err_count != max_errors {
												...
												// Remove entry to clear peer record incase it
												// connects back and becomes responsive
												network_info
													.ping
													.manager
													.outbound_errors
										...
										PingErrorPolicy::DisconnectAfterMaxTimeouts(
											...
											// Disconnect after we've recorded a certain number of
											// concurrent TIMEOUT errors

											// First make sure we're dealing with only the timeout
											// errors
											if let Failure::Timeout = err_type {
												// Get peer entry for outbound errors or initialize
												// peer
												let err_count = network_info
													.ping
													.manager
