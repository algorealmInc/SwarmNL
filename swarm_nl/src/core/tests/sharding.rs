...
#[tokio::test]
async fn join_and_exit_hash_based_shard_network() {

	// Initialize the hash-based sharding policy
	let shard_exec = HashSharding;
	...
	// Ports
	let ports_1: (Port, Port) = (49155, 54203);
	let ports_2: (Port, Port) = (49153, 54201);
	let ports_3: (Port, Port) = (49154, 54202);
	...
	let task_1 = tokio::task::spawn(async move {
		...
		// Join first shard network
		let _ = sharding_executor
			.join_network(node.clone(), &shard_id_1.to_string())
			.await;
		...
		// Exit shard network
		let _ = sharding_executor
			.exit_network(node.clone(), &shard_id_1.to_string())
			.await;
	});
	...
	let task_2 = tokio::task::spawn(async move {
		...
		// Join second shard network
		let _ = sharding_executor
			.join_network(node.clone(), &shard_id_2.to_string())
			.await;
		...
		// Exit shard network
		let _ = sharding_executor
			.exit_network(node.clone(), &shard_id_2.to_string())
			.await;
	});
	...
	let task_3 = tokio::task::spawn(async move {
        ...
        // We expect this node to have no shard before joining 
		let shard_network_state = HashSharding::network_state(node.clone()).await;
        assert_eq!(shard_network_state.len(), 0);

		// Join shard network
		let _ = sharding_executor
			.join_network(node.clone(), &shard_id_3.to_string())
			.await;

        // Now we expect that this node has 1 shard as well as the other nodes
        assert_eq!(shard_network_state.len(), 1);
