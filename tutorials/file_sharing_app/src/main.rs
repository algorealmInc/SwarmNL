...
pub const KADEMLIA_KEY: &str = "bootstrap_config.ini"; // File name
pub const KADEMLIA_VALUE: &str = "bootstrap_config.ini"; // Location on fs (it is in the same directory as our binary)
...
pub const NODE_1_WAIT_TIME: u64 = 3;

/// Node 2 wait time (for node 1 to write to the DHT).
pub const NODE_2_WAIT_TIME: u64 = 5;
...
pub async fn setup_node_1(ports: (Port, Port)) -> Core<FileServer> {
...
pub async fn setup_core_builder_1(buffer: &mut [u8], ports: (u16, u16)) -> Core<FileServer> {
	let app_state = FileServer;
...
async fn run_node_2() {
	...
	if let Ok(result) = node_2.query_network(kad_request).await {
		...
		if let AppResponse::KademliaLookupSuccess(value) = result {
			println!("We will be querying this file from the remote: {}", String::from_utf8_lossy(&value));
			// Now prepare an RPC query to fetch the file from the remote node
			let fetch_key = vec![vec![0]];
			...
			println!("A fetch request has been sent to peer: {:?}", node_1_peer_id);

			node_2.recv_from_network(stream_id).await.unwrap();
